...
public class LigneCommandeService  implements IDao<LigneCommande> {
    ...
    public List<LigneCommande> findAll() {
     ...
     public List<LigneCommande> findBetweenDate(Date d1, Date d2) {
        List<LigneCommande> ligneCommandes = null;
        Session session = null;
        Transaction tx = null;
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            tx = session.beginTransaction();
            ligneCommandes  =  session.getNamedQuery("findBetweenDates").setParameter("d1", d1).setParameter("d2", d2).list();
            tx.commit();
        } catch (HibernateException e) {
            if(tx != null)
                tx.rollback();
